...
export default function Actual({ text }) {
  ...
  //   const modes = [diffChars, diffWords, diffLines, diffSentences];

  let handlechange = (eve) => {
    // let res = modes[mode](text, eve);
    let res = diffChars(text, eve);

    if (mode === "diffWords") {
      res = diffWords(text, eve);
    } else if (mode === "diffLines") {
      res = diffLines(text, eve);
    } else if (mode === "diffSentences") {
      res = diffSentences(text, eve);
    }
